      ...
      {selectedIdx !== null && (
        <div className={styles.overlay} onClick={() => { setSelectedIdx(null); setShowForm(false); }}>
          <div className={styles.expandedCard} onClick={e => e.stopPropagation()}>
            ...
            {!showForm ? (
              <>
                <button style={{padding: '0.7rem 2rem', background: '#f59e42', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: '1.1rem', cursor: 'pointer'}} onClick={() => setShowForm(true)}>Buy Now</button>
