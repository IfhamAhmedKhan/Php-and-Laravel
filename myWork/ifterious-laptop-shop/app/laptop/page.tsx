...
const laptopItems = [
  ...
  },
  {
    name: "Laptop Zbook",
    price: "$1500",
    image: "https://wise-tech.com.pk/wp-content/uploads/2024/08/HP-ZBook-15-G5-Front.png",
    details: "Intel i7, 32GB RAM, Quodro P2000, 15.6'' QHD",
    category: "Laptop",
  },
  {
    name: "Business Pro 15 Laptop",
    price: "$1100",
    image: "https://static.webx.pk/files/4012/Images/71cdlw1zczl.-ac-sl1500--4012-1894764-281023010922152.jpg",
    details: "Intel i7, 16GB RAM, 1TB SSD, 15.6'' FHD",
    category: "Laptop",
  },
  {
    name: "Student Lite Laptop",
    price: "$650",
    image: "https://images.unsplash.com/photo-1517336714731-489689fd1ca8?auto=format&fit=crop&w=400&q=80",
  ...
  {
    name: "Designer Touch Laptop",
    price: "$1350",
    image: "https://m.media-amazon.com/images/I/81fina4As1L._UF894,1000_QL80_.jpg",
    ...
    try {
      const res = await fetch('/api/submit-form', {
        ...
        body: JSON.stringify({
          ...
          productName: selectedIdx !== null ? laptopItems[selectedIdx].name : 'Laptop Product',
          productPrice: selectedIdx !== null ? laptopItems[selectedIdx].price : 'Contact for pricing',
          productDetails: selectedIdx !== null ? laptopItems[selectedIdx].details : '',
      ...
      {selectedIdx !== null && (
        <div className={styles.overlay} onClick={() => { setSelectedIdx(null); setShowForm(false); }}>
          <div className={styles.expandedCard} onClick={e => e.stopPropagation()}>
            <img src={laptopItems[selectedIdx].image} alt={laptopItems[selectedIdx].name} style={{width: '100%', height: 220, objectFit: 'cover', borderRadius: 12, marginBottom: 16}} />
            <h2>{laptopItems[selectedIdx].name}</h2>
            <p className={styles.price} style={{fontSize: '1.4rem'}}>{laptopItems[selectedIdx].price}</p>
            <p style={{margin: '1rem 0'}}>{laptopItems[selectedIdx].details}</p>
            {!showForm ? (
              <>
                <button style={{padding: '0.7rem 2rem', background: '#f59e42', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: '1.1rem', cursor: 'pointer'}} onClick={handleBuyNow}>Buy Now</button>
